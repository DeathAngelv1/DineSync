...
class DineSyncAIEngine:
    ...
    def train_models(self):
        ...
        try:
            ...
            logger.info("AI Wait Time Regressor successfully trained on historical data!")
    ...
    def predict_wait_time(self, party_size: int, preferred_section: str = "Any", target_hour: Optional[int] = None, day_of_week: Optional[int] = None) -> Dict[str, Any]:
        """Dynamic AI wait time estimation taking current restaurant state into account"""
        now = datetime.now()
        ...
        # If matching table is immediately available and no queue ahead, 0-2 mins wait!
        if active_queue_len == 0 and matching_available > 0:
            return {
